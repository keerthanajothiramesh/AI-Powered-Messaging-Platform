...
from src.messaging.schemas import CreateGroupRequest
...
async def create_group(data: CreateGroupRequest, current_user=Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                "INSERT INTO groups (group_id, group_name, description, created_by) VALUES ($1,$2,$3,$4)",
    ...
    group_event = {
        "type": "group_added",
        "data": {"group_id": group_id, "group_name": data.group_name,
                 "description": data.description, "member_count": len(data.member_ids) + 1},
    }
...
async def my_groups(current_user=Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """SELECT g.group_id, g.group_name, g.description, g.avatar_url,
                      g.created_by, g.created_at, COUNT(gm2.user_id) as member_count
               FROM groups g
    ...
    return [
        {
            "group_id": str(r["group_id"]), "group_name": r["group_name"],
            "description": r["description"], "avatar_url": r["avatar_url"],
            "created_by": str(r["created_by"]), "member_count": r["member_count"],
...
async def get_group(group_id: str, current_user=Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        ...
        members = await conn.fetch(
            ...
            "FROM group_members gm JOIN users u ON gm.user_id = u.user_id WHERE gm.group_id=$1",
            group_id,
        )
    return {
        "group_id": str(group["group_id"]), "group_name": group["group_name"],
        "description": group["description"], "avatar_url": group["avatar_url"],
        "created_by": str(group["created_by"]),
        "created_at": group["created_at"].isoformat() if group["created_at"] else None,
        "members": [
            {"user_id": str(m["user_id"]), "display_name": m["display_name"],
             "user_presence": m["user_presence"], "avatar_url": m["avatar_url"], "role": m["role"]}
            for m in members
        ],
    }
