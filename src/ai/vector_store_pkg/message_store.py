...
class MessageVectorMixin:
    """Handles per-message embedding upsert and similarity search."""
            ...
            async with self._pool.acquire() as conn:
                await conn.execute(
                    """INSERT INTO message_embeddings
                       (message_id, embedding, content, sender_id, group_id,
                        receiver_id, media_type, language)
                       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                       ON CONFLICT (message_id) DO NOTHING""",
                    message_id, embedding, content,
                    metadata.get("sender_id", ""), metadata.get("group_id", ""),
                    metadata.get("receiver_id", ""), metadata.get("media_type", "text"),
                    metadata.get("language", "en"),
                )
        except Exception as exc:
            logger.warning("pgvector_add_failed", message_id=message_id, error=str(exc))
    ...
    async def search_similar(
        ...
        sql = f"""
            ...
            FROM message_embeddings {where_sql}
        ...
        try:
            ...
            return [
                {
                    "message_id": r["message_id"], "content": r["content"],
                    "score": float(r["score"]),
                    "metadata": {
                        "sender_id": r["sender_id"], "group_id": r["group_id"],
                        "receiver_id": r["receiver_id"], "media_type": r["media_type"],
                        "language": r["language"],
                    },
                }
                for r in rows
