...
- Use bullet points for clarity""",
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = await summarise_conversation(group_id, days=days, group_name=group_name)

        from src.agents.judge_agent import JudgeAgent
        judge = JudgeAgent()
        judgment = await judge.evaluate(summary, input_data)

        if judgment.get("average_score", 10) < 7:
            logger.warning("summary_quality_low", score=judgment["average_score"])
            refined_prompt = f"""The previous summary scored {judgment['average_score']}/10.
Issues: {judgment.get('feedback', '')}
Please generate an improved summary for {group_name} addressing these issues."""
